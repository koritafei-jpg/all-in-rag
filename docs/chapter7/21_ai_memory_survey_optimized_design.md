...
| 框架名称 | 现有缺陷 | 对应创新设计 |
| --- | --- | --- |
| Mem0 $^{[1]}$ | 最新开源版图关系能力弱化，缺乏系统化主动遗忘 | 轻量内存图 + 结构化字段；内置艾宾浩斯遗忘$^{[2]}$与主动修剪 |
...
*   图分：`GraphScore_i` 来自路径强度与证据覆盖（第 7 章详述）。  
*   默认权重：`w_v=0.45, w_k=0.30, w_t=0.10, w_imp=0.05, w_g=0.10`，后续由自进化控制引擎调优。  

### 5.2 记忆强度更新

$S_i <- S_i + \lambda * reinforce_count$  
$TimeDecay_i = exp(-\Delta t / S_i)$

初始 `S_i(0)=1`，`lambda=0.5`。每次有效回忆或正向反馈强化记忆强度，受 SCM/SuperLocalMemory 启发$^{[7][9]}$。
...
[23] Qdrant Vector Database. https://qdrant.tech/  
